...
module.exports = client => {
    ...
    app.get("/v1/invite/_callback", (req, res) => {
        res.redirect("http://82.165.61.240:3000" + config.website.invite);
    });

    app.get("/v1/invite/bot", (req, res) => {
        if (!req.query["__w"]) return res.redirect(config.auth.discord.botInvite + (req.query["__beta"] === "true" ? ("&redirect_uri=http://82.165.61.240:3000/dashboard/added") : "&redirect_uri=https://awardbot.me/dashboard/added") + (req.query["disable_select"] ? ("&disable_guild_select=true") : "") + (req.query["id"] ? ("&guild_id=" + req.query["id"]) : ""));
        res.redirect(config.auth.discord.botInvite + (req.query["__beta"] === "true" ? "&redirect_uri=https://giftix-api.netlify.app/v1/invite/_callback" : "&redirect_uri=https://giftix-api.netlify.app/v1/invite/callback") + (req.query["disable_select"] ? ("&disable_guild_select=true") : "") + (req.query["id"] ? ("&guild_id=" + req.query["id"]) : ""));
    });
