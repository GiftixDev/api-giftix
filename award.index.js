...
const updateActions = async ({ channel = false, title, color, guild = [] }) => {
  ...
  client.user.setActivity("/help | giftix.eu | " + client.guilds.cache.size.toLocaleString() + " guilds", { type: "WATCHING" });
...
client.on("guildCreate", (guild) => updateActions({ channel: '949952275470766080', title: 'New guild', color: 'GREEN', guild: guild }));
client.on("guildDelete", (guild) => updateActions({ channel: '949952275470766080', title: 'Guild deleted', color: 'RED', guild: guild }));
...
client.on("guildCreate", guild => {
  try {
    if (guild.memberCount < 20) {
      if ([
        "948295562137071656", // Support Server
...
client.on("interaction", async interaction => {
  try {
    ...
    const _request = await axios.post("https://giftix-api.netlify.app/v1/giveaway/" + interaction.customId.replace("join-", "") + "/join?_token=" + _profile.token).catch(() => { });
